...
import { 
  ...
  ChevronRight, AlertCircle as AlertCircleIcon, Heart, Droplets
} from 'lucide-react'
import {
...
import { 
  ...
  // Dairy might need specific health/feed too, usually shared or fallback
} from '@/lib/hooks/useKambingPerahData'

import { getLivestockConfig } from '@/lib/constants/taskTemplates'

// ── KAMBING PERAH SPECIFIC CONFIG ─────────────────────────────────────────────

const KAMBING_PERAH_HOOKS = {
  useAnimals: useKambingPerahAnimals,
  useAddWeight: () => ({ mutateAsync: () => { toast.error('Timbang tidak didukung di modul perah'); return Promise.reject() } }),
  useBatches: () => ({ data: [], isLoading: false }), 
  useActiveBatches: () => ({ data: [], isLoading: false }),
  useAddHealth: () => ({ mutateAsync: () => { toast.error('Log kesehatan tidak didukung di modul perah via tugas'); return Promise.reject() } }),
  useAddFeed: () => ({ mutateAsync: () => { toast.error('Log pakan tidak didukung di modul perah via tugas'); return Promise.reject() } }),
  useLogProduction: useLogKambingPerahProduction,
}
...
export default function KambingPerahDailyTask() {
  const livestockType = 'kambing_perah'
