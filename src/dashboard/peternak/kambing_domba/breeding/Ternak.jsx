...
import {
  ...
  useAddKdBreedingAnimal,
  useAddKdBreedingWeight,
  calcAgeInDays,
...
function AddAnimalSheet({ open, onClose, animals }) {
  const addAnimal = useAddKdBreedingAnimal()
...
function TimbangSheet({ animal, onClose }) {
  const addWeight = useAddKdBreedingWeight()
  const [form, setForm] = useState({
...
export default function BreedingTernak() {
  const { data: animals = [], isLoading } = useKdBreedingAnimals()
