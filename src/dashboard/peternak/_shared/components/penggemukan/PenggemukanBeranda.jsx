        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 px-4 mt-4">
          ...
          <div className="col-span-2 sm:col-span-1">
            <KPICard
              ...
              sub={parseFloat(kpi.mortalitasPct) > mortalityThreshold ? `🚨 Di atas batas ${mortalityThreshold}%` : parseFloat(kpi.mortalitasPct) > 0 ? '⚠ Dipantau' : '✓ Aman'}
              icon={Activity}
              color={parseFloat(kpi.mortalitasPct) > mortalityThreshold ? 'text-red-400' : parseFloat(kpi.mortalitasPct) > 0 ? 'text-amber-400' : 'text-green-400'}
              glow={parseFloat(kpi.mortalitasPct) > mortalityThreshold ? 'red' : parseFloat(kpi.mortalitasPct) > 0 ? 'amber' : 'green'}
            />
          </div>
        </div>
      )}
            ...
            {activeBatches.length === 0 ? (
              <div className="relative group overflow-hidden rounded-[2rem] border border-white/10">
                ...
                <div className="relative z-10 text-center py-16 px-6">
                  ...
                  <p className="text-xs text-[#4B6478] mb-8 max-w-[240px] mx-auto leading-relaxed">
                    Mulai monitoring penggemukan {animalLabel.toLowerCase()} dengan membuat batch pertama sekarang.
                  </p>
                  <button
                    onClick={() => navigate(`${BASE}/batch`)}
                  ...
                  >
                    ...
                    <div className="border-t border-amber-500/10 px-4 pb-3 pt-2 space-y-1.5">
                      {a.pendingTasks.map((t, ti) => {
                        const title = t.template?.title || 'Tugas'
