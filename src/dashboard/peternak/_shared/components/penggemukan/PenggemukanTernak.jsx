...
/**
 * PenggemukanTernak — shared fattening animal list page
 *
 * Props:
 *   config  — { BASE, animalLabel, breedSuggestions, limitType, searchPlaceholder,
 *               weightRecordsKey? }
 *             weightRecordsKey: species-specific weight records field name.
 *             Defaults to 'weight_records' (already normalized by the hook factory).
 *             Pass e.g. 'sapi_penggemukan_weight_records' for hooks that return raw DB data.
 *   hooks   — { useBatches, useAnimals, useAnimalsByBatches, useAddAnimal,
 *               useUpdateAnimal, useBulkAddAnimals, useAddSale, useAddWeightRecord }
 */
export function PenggemukanTernak({ config, hooks }) {
  const {
    BASE,
    animalLabel,
    breedSuggestions,
    limitType,
    searchPlaceholder,
    weightRecordsKey = 'weight_records',
  } = config
  ...
  // ── Animals ─────────────────────────────────────────────────────────────────
  const { data: allAnimals    = [], isLoading: loadingAll    } = useAnimalsByBatches(isAllBatches ? allBatchIds : [])
  const { data: singleAnimals = [], isLoading: loadingSingle } = useAnimals(isAllBatches ? null : selectedBatchId)
  const rawAnimals     = isAllBatches ? allAnimals : singleAnimals
  const loadingAnimals = isAllBatches ? loadingAll : loadingSingle

  // Normalize to _weightRecords for shared components (AnimalCard, AnimalDetailSheet, QuickWeighSheet).
  // Hook factory already normalizes species-specific field to `weight_records`.
  // For hooks that return raw DB data (e.g. sapi), pass weightRecordsKey in config.
  const animals = useMemo(
    () => rawAnimals.map(a => ({ ...a, _weightRecords: a.weight_records ?? a[weightRecordsKey] ?? [] })),
    [rawAnimals, weightRecordsKey]
  )

  // ── Domain mutations ─────────────────────────────────────────────────────────
  const { mutate: addAnimal,   isPending: addingAnimal   } = useAddAnimal()
  const { mutate: updateAnimal, isPending: updatingAnimal } = useUpdateAnimal()
  const { mutate: bulkAdd,     isPending: bulkAdding     } = useBulkAddAnimals()
  const { mutate: addSale,     isPending: selling        } = useAddSale()
  const { mutate: addWeight,   isPending: weighing       } = useAddWeightRecord()
                  ...
                  <>
                    ...
                    {isAllBatches ? (
                      ...
                      <>
                        ...
                        >
                          <Plus size={14} strokeWidth={3} /> Tambah Ternak
                        </button>
                      </>
                    )}
                  </>
      ...
      <AnimatePresence>
        {sheet === 'add' && (
          <AddAnimalSheet
            ...
            animalLabel={animalLabel} breedSuggestions={breedSuggestions}
