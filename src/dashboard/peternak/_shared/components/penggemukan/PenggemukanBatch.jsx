...
function BatchCard({ batch, onClick, onCloseBatch, config, KandangMiniMap }) {
  const perm = usePeternakPermissions()
  const hari   = config.calcHariDiFarm(batch.start_date, batch.end_date)
  const TARGET_HARI = config.targetDays
  const sisaHari = Math.max(0, TARGET_HARI - hari)
  const estimasiPanen = addDays(new Date(batch.start_date), TARGET_HARI)

  const mort   = config.calcMortalitas(batch.mortality_count, batch.total_animals)
      ...
      <div className={cn("flex flex-col gap-6", batch.status === 'active' && "lg:flex-row")}>
        ...
        <div className="flex-1 flex flex-col justify-between min-w-0">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="bg-white/[0.02] border border-white/[0.04] rounded-2xl p-2.5 text-center">
              <p className={`text-[11px] font-black leading-none mb-1 ${mort > config.mortalityThreshold ? 'text-red-400' : 'text-green-400'}`}>
                {mort}%
              </p>
        ...
        {/* Right Side: Map Visualization (Only for Active, only when KandangMiniMap is provided) */}
        {batch.status === 'active' && KandangMiniMap && (
        ...
        {kpi.alive_count > 0 && (
          <div className="bg-amber-500/5 border border-amber-500/20 rounded-2xl p-4 flex gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-[#4B6478] mt-0.5 leading-relaxed">{config.animalLabel} yang belum terjual statusnya akan tetap &apos;aktif&apos; setelah batch ditutup. Pastikan sudah dicatat dengan benar.</p>
            </div>
          </div>
        )}
...
function CreateBatchSheet({ config, hooks, onClose }) {
  ...
  const { mutate: createBatch, isPending } = hooks.useCreateBatch()
...
export function PenggemukanBatch({ config, hooks, KandangMiniMap }) {
  const navigate = useNavigate()
  const perm = usePeternakPermissions()
  const [showCreate, setShowCreate] = useState(false)
  const [closingBatch, setClosingBatch] = useState(null)
  const [search, setSearch] = useState('')
  const { data: batches = [], isLoading } = hooks.useBatches()
            ...
            <div className="space-y-6">
              {active.map(b => (
                <BatchCard
                  ...
                  batch={b}
                  config={config}
                  KandangMiniMap={KandangMiniMap}
                  onClick={() => navigate(`${config.BASE}/ternak?batch=${b.id}`)}
            ...
            <div className="space-y-6">
              {closed.map(b => (
                <BatchCard
                  ...
                  config={config}
                  KandangMiniMap={KandangMiniMap}
                  onClick={() => navigate(`${config.BASE}/laporan?batch=${b.id}`)}
                />
              ))}
            </div>
          </section>
      ...
      <AnimatePresence>
        {showCreate && (
          <>
            ...
            />
            <CreateBatchSheet config={config} hooks={hooks} onClose={() => setShowCreate(false)} />
          </>
        )}
        {closingBatch && (
          <>
            ...
            />
            <CloseBatchWizard batch={closingBatch} config={config} hooks={hooks} onClose={() => setClosingBatch(null)} />
          </>
        )}
      </AnimatePresence>
    </div>
