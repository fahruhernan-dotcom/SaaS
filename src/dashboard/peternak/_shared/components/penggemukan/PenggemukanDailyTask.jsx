...
/**
 * Shared DailyTask page for all fattening livestock types.
 *
 * Props:
 *   livestockType           — "domba_penggemukan" | "kambing_penggemukan" | "sapi_penggemukan"
 *   hooks                   — { useAnimals, useAddWeight, useBatches, useActiveBatches, useAddHealth, useAddFeed, weightTable }
 *   renderExtraReportFields — optional (context, data, setData) => ReactNode
 */
export function PenggemukanDailyTask({ livestockType, hooks, renderExtraReportFields }) {
  ...
  const tasks = useMemo(() => {
    ...
    if (auditRange === 'week') {
      ...
      return monthTasks.filter(t => { const d = parseISO(t.due_date); return d >= start && d <= end })
    }
    if (auditRange === 'month') {
  ...
  const itemVariants = {
    ...
    visible: { opacity: 1, x: 0, scale: 1, transition: { type: 'spring', damping: 22, stiffness: 120 } },
  }
            ...
            <GlassCard className="rounded-[40px] bg-white/[0.01]">
              <div className="p-7 border-b border-white/5 flex items-center justify-between">
                ...
                <div className="w-10 h-10 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10">
                  <CalendarIcon size={16} className="text-[#A78BFA]" />
                </div>
              </div>
              <div className="p-8">
                <CustomCalendar
