...
export default function UniversalTaskSettings({ livestockType = 'sapi_penggemukan' }) {
  ...
  // Data
  const { data: templates = [], isLoading } = usePeternakTaskTemplates({ livestockType })
...
function TemplateFormSheet({ open, onOpenChange, template, isDesktop, livestockType }) {
  ...
  function handleSubmit() {
    ...
    if (isEdit) {
      updateTask.mutate({ id: template.id, ...form, livestock_type: livestockType }, { onSuccess: () => onOpenChange(false) })
    } else {
      createTask.mutate({ ...form, livestock_type: livestockType }, { onSuccess: () => onOpenChange(false) })
    }
  }
...
function TernakOSTemplateSheet({ open, onOpenChange, existingTemplates, isDesktop, livestockType }) {
  const [selectedPackage, setSelectedPackage] = useState(null)
  const [selectedBatchId, setSelectedBatchId] = useState('')
  
  const config = getLivestockConfig(livestockType)
  const defaultTemplates = config.defaultTemplates || {}

  const packageInfo = useMemo(() => {
    return Object.entries(defaultTemplates).map(([name, tasks]) => ({
      id: name,
      label: `Paket ${name}`,
      desc: `Program ${config.animalLabel} standar dengan siklus pakan dan medis terstruktur.`,
      adg: livestockType.includes('sapi') ? '~1 kg/hari' : '~200g/hari',
      count: tasks.length,
    }))
  }, [livestockType, defaultTemplates, config.animalLabel])
          ...
          <SheetHeader className="text-left space-y-2">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-[#4B6478] font-medium mt-0.5">Berdasarkan standar manajemen {config.label}</p>
              </div>
            </div>
          </SheetHeader>
