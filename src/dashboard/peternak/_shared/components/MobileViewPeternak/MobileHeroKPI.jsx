...
function MetricCell({ label, value, unit, tone, divider, sparkData, sparkColor }) {
  ...
  return (
    <div className={`py-3 px-3.5 flex flex-col gap-1 ${divider ? 'border-l border-white/[0.03]' : ''}`}>
...
export function MobileHeroKPI({
  ...
  return (
    <motion.div
      ...
      className="bg-white/[0.03] border border-white/[0.03] rounded-[1.5rem] overflow-hidden shadow-xl"
    >
      ...
      {/* Bottom metric row */}
      <div className={`grid ${gridColsClass} border-t border-white/[0.03]`}>
        {cells}
      </div>
    </motion.div>
  )
