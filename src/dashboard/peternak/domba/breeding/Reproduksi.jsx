...
import {
  ...
  useAddDombaBreedingMating,
  useUpdateDombaBreedingMating,
  useAddDombaBreedingBirth,
...
function AddMatingSheet({ open, onClose, animals }) {
  const addMating = useAddDombaBreedingMating()
...
function ConfirmPregSheet({ mating, onClose }) {
  const updateMating = useUpdateDombaBreedingMating()
  const [form, setForm] = useState({
...
function RecordBirthSheet({ mating, onClose }) {
  const addBirth = useAddDombaBreedingBirth()
  const [form, setForm] = useState({
...
export default function BreedingReproduksi() {
  const { data: matings = [], isLoading: loadM } = useDombaBreedingMatings()
  const { data: births  = [], isLoading: loadB } = useDombaBreedingBirths()
  const { data: animals = [] } = useDombaBreedingAnimals()
