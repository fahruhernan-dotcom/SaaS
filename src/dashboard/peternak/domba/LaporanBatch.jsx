...
import {
  useDombaBatches, useDombaAnimals, useDombaFeedLogs, useDombaSales,
  calcHariDiFarm, calcADG, calcFCRDomba, calcMortalitasDomba,
  calcBEP, calcRCRatio,
...
export default function KdPenggemukanLaporan() {
  ...
  const { data: feedLogs = [], isLoading: loadingFeed     } = useDombaFeedLogs(selectedBatch)
  const { data: sales = [],    isLoading: loadingSales    } = useDombaSales(selectedBatch)
  ...
  const kpi = useMemo(() => {
    ...
    const fcr = calcFCRDomba(totalPakan, totalPBBH)
