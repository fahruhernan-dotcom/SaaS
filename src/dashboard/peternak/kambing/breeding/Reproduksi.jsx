...
import {
  useKambingBreedingMatings,
  useKambingBreedingBirths,
  useKambingBreedingAnimals,
  useAddKambingBreedingMating,
...
function AddMatingSheet({ open, onClose, animals }) {
  const addMating = useAddKambingBreedingMating()
...
function ConfirmPregSheet({ mating, onClose }) {
  const updateMating = useUpdateKambingBreedingMating()
  const [form, setForm] = useState({
...
function RecordBirthSheet({ mating, onClose }) {
  const addBirth = useAddKambingBreedingBirth()
  const [form, setForm] = useState({
...
export default function BreedingReproduksi() {
  const { data: matings = [], isLoading: loadM } = useKambingBreedingMatings()
  const { data: births  = [], isLoading: loadB } = useKambingBreedingBirths()
  const { data: animals = [] } = useKambingBreedingAnimals()
