...
export default function KandangView() {
  ...
  const { data: batches = [], isLoading: loadingBatches } = useKambingActiveBatches()
  ...
  // Data Hooks
  const { data: kandangs = [], isLoading: loadKdg } = useKambingKandangs()
  const { data: animals = [], isLoading: loadAni } = useKambingAnimals(selectedBatch)
  const ensureHoldingPen = useEnsureKambingHoldingPen()

  useEffect(() => {
    if (selectedBatch) ensureHoldingPen.mutate()
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedBatch])

  const createKandang = useCreateKambingKandang()
  const moveAnimal = useMoveKambingToKandang()
