...
export default function SapiPakan() {
  ...
  const { data: logs = [], isLoading: loadingLogs } = useSapiFeedLogs(activeBatch?.id)
  const addLog = useAddSapiFeedLog()
  const deleteLog = useDeleteSapiFeedLog()
    ...
    <div className="text-slate-100 pb-24">
      ...
      <header className="px-4 pt-6 pb-5 bg-gradient-to-b from-[#0C1319] to-[#06090F] border-b border-white/[0.04]">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="font-['Sora'] font-black text-xl text-white">Log Pakan Sapi</h1>
        </div>
        ...
        {batches.length > 0 && (
          <div className="flex gap-2 overflow-x-auto no-scrollbar py-1">
            {batches.map(b => (
              <button
                key={b.id}
                onClick={() => navigate(`${BASE}/stok-pakan?batch=${b.id}`)}
      ...
      {!activeBatch ? (
        ...
        <div className="px-4 mt-6">
          ...
          <div className="grid grid-cols-2 gap-3 mb-8">
            <div className="bg-amber-500/[0.05] border border-amber-500/10 rounded-2xl p-4">
              <p className="text-[10px] text-amber-500/60 font-bold uppercase tracking-wider mb-1">Total Konsumsi</p>
          ...
          <div className="fixed inset-0 z-[100] flex items-end justify-center sm:items-center sm:p-4">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-4 font-['DM Sans']">
                <div>
                  <label className="block text-[10px] font-bold text-[#4B6478] uppercase mb-1.5 ml-1">Tanggal Pemberian</label>
                  <input
                    type="date"
                    required
                    value={form.log_date}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-bold text-[#4B6478] uppercase mb-1.5 ml-1">Hijauan (kg)</label>
                    <input
                      type="number"
                      step="0.1"
                      placeholder="0.0"
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-[#4B6478] uppercase mb-1.5 ml-1">Konsentrat (kg)</label>
                    <input
                      type="number"
                      step="0.1"
                      placeholder="0.0"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-bold text-[#4B6478] uppercase mb-1.5 ml-1">Dedak (kg)</label>
                    <input
                      type="number"
                      step="0.1"
                      placeholder="0.0"
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-[#4B6478] uppercase mb-1.5 ml-1">Sisa Pakan (kg)</label>
                    <input
                      type="number"
                      step="0.1"
                      placeholder="0.0"
                      ...
                      className="w-full bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-3 text-sm text-amber-400 placeholder:text-[#4B6478] focus:outline-none focus:border-amber-500/50 font-bold"
                    />
                  </div>
                </div>

                <div>
                  <label className="block text-[10px] font-bold text-[#4B6478] uppercase mb-1.5 ml-1">Lainnya (kg)</label>
                  <input
                    type="number"
                    step="0.1"
                    placeholder="0.0"
                ...
                <div>
                  <label className="block text-[10px] font-bold text-[#4B6478] uppercase mb-1.5 ml-1">Catatan</label>
                  <textarea
                    placeholder="Catatan pakan..."
                    rows={2}
                    value={form.notes}
                    onChange={e => setForm({...form, notes: e.target.value})}
                    className="w-full bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-3 text-sm text-white placeholder:text-[#4B6478] focus:outline-none focus:border-amber-500/50 resize-none"
                  />
                </div>
