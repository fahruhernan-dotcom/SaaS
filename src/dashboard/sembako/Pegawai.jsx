...
import {
  Users, Plus, DollarSign, CalendarCheck, Check, Briefcase, Phone, ChevronDown,
} from 'lucide-react'
import {
...
function TabPayroll({ isDesktop }) {
  ...
  // Sync derived values
  useMemo(() => { setBaseAmount(calcBase) }, [calcBase])
  useMemo(() => { setCommissionAmount(calcComm) }, [calcComm])

  const totalPay = baseAmount + commissionAmount + bonus - deduction
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ background: C.card, borderRadius: '16px', padding: '20px', border: `1px solid ${C.border}` }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isDesktop ? '1fr 1fr' : '1fr', gap: '12px' }}>
          <div>
            ...
            <SelectWrap>
              <select style={{ ...sInput, paddingRight: 32 }} value={empId} onChange={e => handleSelectEmp(e.target.value)}>
                <option value="">— Pilih pegawai —</option>
                {employees.filter(e => e.status === 'aktif').map(e => (
                  <option key={e.id} value={e.id}>{e.full_name} ({e.salary_type})</option>
                ))}
              </select>
            </SelectWrap>
          </div>
          <div>
            ...
            <input type="date" style={sInput} value={periodDate} onChange={e => setPeriodDate(e.target.value)} />
          </div>
        </div>
          ...
          <div style={{ marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ background: C.input, borderRadius: '12px', padding: '14px', border: `1px solid ${C.border}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                ...
                <span style={{ fontSize: '12px', color: C.text, fontWeight: 600 }}>{formatIDR(baseAmount)}</span>
              </div>
              {commissionAmount > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                <span style={{ fontSize: '11px', color: C.muted }}>Komisi</span>
                <span style={{ fontSize: '12px', color: C.text, fontWeight: 600 }}>+ {formatIDR(commissionAmount)}</span>
              </div>}
      ...
      <div>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '12px', flexWrap: 'wrap' }}>
          ...
          <SelectWrap style={{ width: 'auto', minWidth: '160px' }}>
            <select style={{ ...sInput, width: 'auto', minWidth: '160px', paddingRight: 32 }} value={filterEmp} onChange={e => setFilterEmp(e.target.value)}>
