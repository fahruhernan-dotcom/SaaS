    ...
    <div className={cn("bg-[#06090F] min-h-screen pb-24 text-left", isDesktop ? "p-10 space-y-10" : "p-5 pt-10 space-y-8")}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <Card className={cn("bg-[#111C24] border-white/5 rounded-[32px] relative overflow-hidden", isDesktop ? "lg:col-span-2 p-8" : "p-6")}>
                ...
                {loadingInv ? [1,2,3].map(i => <Skeleton key={i} className="h-16 w-full rounded-2xl bg-white/5" />) :
                 inventory?.length === 0 ? <p className="text-xs font-bold text-[#4B6478] italic">Belum ada data inventori</p> :
                  inventory?.slice(0, 5).map(item => (
                    <div key={item.id} className="flex justify-between items-center p-4 bg-white/[0.03] rounded-2xl border border-white/5 group hover:border-emerald-500/30 transition-all cursor-pointer" onClick={() => navigate('/broker/inventori')}>
            ...
            {inventory?.length > 5 && (
                <Button variant="ghost" className="w-full mt-4 text-[#4B6478] font-black uppercase text-[10px] tracking-widest gap-2" onClick={() => navigate('/broker/inventori')}>
        ...
        <Card className={cn("bg-[#111C24] border-white/5 rounded-[32px] space-y-6", isDesktop ? "p-8" : "p-6")}>
            ...
            <NavCard 
                ...
                icon={Building2} 
                onClick={() => navigate('/broker/suppliers')}
            ...
            <NavCard 
                ...
                icon={Users} 
                onClick={() => navigate('/broker/customers')}
                color="text-blue-400"
            />
            <NavCard 
                title="Transaksi" 
