...
export default function CashFlow() {
    ...
    const { chartData, breakdownData, totalTransport, totalSusutValue } = useMemo(() => {
        ...
        let actualStart = dateRange.start;
        let actualEnd = dateRange.end;
        if (selectedPeriod === 'all') {
            const allDates = [
                ...sales.map(s => new Date(s.transaction_date)), 
                ...purchases.map(p => new Date(p.transaction_date)), 
                ...expenses.map(e => new Date(e.expense_date))
            ].sort((a,b) => a-b);
            
            if (allDates.length > 0) {
                actualStart = allDates[0];
                actualEnd = new Date();
            } else {
        ...
        // Pre-group for O(N) chart generation
        const salesByDate = {}
        sales.forEach(s => {
            if(!salesByDate[s.transaction_date]) salesByDate[s.transaction_date] = []
            salesByDate[s.transaction_date].push(s)
        })
        const expensesByDate = {}
        expenses.forEach(e => {
            if(!expensesByDate[e.expense_date]) expensesByDate[e.expense_date] = []
            expensesByDate[e.expense_date].push(e)
        })
        ...
        const cData = days.map(day => {
            ...
            return {
                date: selectedPeriod === 'all' ? format(day, 'dd MMM') : dayLabel,
                fullDate: dateStr,
                                ...
                                <div className="relative w-[180px] h-[180px] flex-shrink-0 overflow-hidden">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <PieChart>
                                            ...
                                            <Tooltip 
                                                contentStyle={{ background: '#0C1319', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '12px', fontSize: '11px', color: '#F1F5F9', fontWeight: 800 }}
                                        ...
                                        <div key={i} className="flex flex-col gap-1">
                                            <div className="flex items-center justify-between text-xs font-bold gap-2">
                                                <div className="flex items-center gap-2 min-w-0 flex-1">
                                                    ...
                                                    <span className="text-white uppercase tracking-wider truncate" title={item.name}>{item.name}</span>
                                                </div>
                                                <span className="text-white tabular-nums flex-shrink-0">{formatIDRShort(item.value)}</span>
                                            </div>
                                        ...
                                        <div className="flex flex-col gap-1 pt-4 border-t border-white/5 mt-2">
                                            <div className="flex items-center justify-between text-xs font-bold gap-2">
                                                <div className="flex items-center gap-2 min-w-0 flex-1">
                                                    ...
                                                    <span className="text-[#F59E0B] uppercase tracking-wider truncate" title="Susut Berat">Susut Berat</span>
                                                </div>
                                                <span className="text-[#F59E0B] tabular-nums flex-shrink-0">{formatIDRShort(totalSusutValue || 0)}</span>
                                            </div>
