...
export default function SembakoTimManajemenPage() {
  ...
  const TABS = [
    { id: 'karyawan', label: 'Karyawan', icon: Users, desc: 'Pekerja & gaji' },
  ...
  return (
    <div className="flex flex-col min-h-full bg-[#06090F]">
      {/* ── Mobile sticky header with tab switcher ─────────────────────────── */}
      {!isDesktop && (
        <header className="sticky top-0 z-30 bg-[#06090F]/90 backdrop-blur-md border-b border-white/5 pt-[env(safe-area-inset-top,0px)]">
          <div className="h-14 px-4 flex items-center gap-3">
            ...
            >
              <Menu size={18} className="text-[#EA580C]" />
            </button>
            {/* Tab pills */}
          ...
          <div className="flex items-end gap-6 border-b border-white/[0.06] mb-0">
            {TABS.map(tab => {
              ...
              return (
                ...
                >
                  <Icon size={15} className={active ? 'text-[#EA580C]' : ''} />
                  {tab.label}
                  {active && tab.desc && (
      ...
      <div className="flex-1 min-h-0">
        {activeTab === 'karyawan' && <Pegawai hideMobileHeader />}
        {activeTab === 'tim' && <Tim hideMobileHeader />}
      </div>
    </div>
  )
}
