...
Discovered during the Phase 6.F audit: `useSapiPenggemukanData.js` is the Sapi-specific hook file that duplicates the penggemukan pattern but is **not** generated by the `createPenggemukanHooks.js` factory. All 6 Kandang/animal-movement mutation sites in this file were missing error instrumentation.

**Sites patched (6 new action_names, prefix `sapi.*`):**
- `sapi.kandang.create` — `useCreateSapiKandang`
- `sapi.animal.move_kandang` — `useMoveSapiAnimalToKandang`
- `sapi.kandang.update_position` — `useUpdateSapiKandangPosition`
- `sapi.kandang.update` — `useUpdateSapiKandang`
- `sapi.kandang.ensure_holding_check` — `useEnsureSapiHoldingPen` (SELECT step)
- `sapi.kandang.ensure_holding_create` — `useEnsureSapiHoldingPen` (INSERT step)

**Prefix rationale:** `sapi.*` (not `peternak.*`) — Sapi penggemukan is larger and more complex than the factory-managed verticals. Vertical-specific prefixes give cleaner observability. Documented as intentional exception in INVENTORY.
