...
| `business_model` | text | `'mandiri_murni'` \| `'mandiri_semi'` \| `'mitra_penuh'` \| `'mitra_pakan'` \| `'mitra_sapronak'` |
| `mitra_company` | text | nullable — nama perusahaan mitra (jika kemitraan) |
...
| `chicken_type` | text | default `'ayam_broiler'` |
| `doc_count` | integer | jumlah DOC masuk |
| `current_count` | integer | ekor hidup sekarang (diupdate tiap InputHarian) |
| `status` | text | `'active'` `'harvested'` `'failed'` `'cancelled'` |
| `start_date` | date | tanggal chick-in |
| `estimated_harvest_date` | date | nullable |
| `total_feed_kg` | numeric | total kumulatif pakan (diupdate tiap InputHarian) |
| `total_mortality` | integer | total kumulatif mortalitas |
| `final_fcr` | numeric | nullable — diisi saat panen |
| `final_ip_score` | numeric | nullable — diisi saat panen |
| `is_deleted` | boolean | default false |

**⚠️ Update pattern**: Setiap `InputHarianSheet.handleSubmit()` UPDATE `current_count`, `total_mortality`, `total_feed_kg`.
...
| `total_weight_kg` | numeric | total bobot panen |
| `total_count` | integer | jumlah ekor yang dipanen |
| `avg_weight_kg` | numeric | nullable — computed dari total_weight/total_count |
| `price_per_kg` | integer | harga jual per kg |
| `net_revenue` | bigint | total pendapatan = total_weight_kg × price_per_kg |
...
| `salary_system` | text | `'flat_bonus'` (satu-satunya nilai saat ini) |
| `base_salary` | integer | gaji pokok per bulan (IDR) |
| `bonus_per_kg` | integer | bonus per kg saat panen (IDR) |
| `fcr_target` | numeric | nullable — FCR threshold untuk dapat bonus |
| `notes` | text | nullable |
...
| `id` | uuid PK | |
| `tenant_id` | uuid FK → tenants | |
| `worker_id` | uuid FK → farm_workers | |
| `payment_date` | date | NOT NULL |
| `payment_type` | text | `'gaji'` \| `'bonus'` \| `'makan'` \| `'lain'` |
| `amount` | bigint | NOT NULL |
| `notes` | text | nullable |

**⚠️ Note**: Tidak ada `is_deleted` dan tidak ada `cycle_id`. Filter untuk laporan menggunakan `worker_id IN [workers farm]` + `payment_date BETWEEN start_date AND harvest_date`.
...
⚠️ Kolom `is_deleted` TIDAK ADA di tabel ini — jangan filter `is_deleted`
...
| `address` | text | nullable |
| `payment_terms` | text | `'cash'` \| `'net3'` \| `'net7'` \| `'net14'` \| `'net30'` |
| `credit_limit` | integer | nullable — batas kredit dalam Rupiah |
| `total_outstanding` | integer | total piutang aktif (update via trigger/manual) |
| `is_deleted` | boolean | |
| `created_at` | timestamptz | |
...
| `qty_keluar` | numeric | |
| `reason` | text | `'sale'` \| `'expired'` \| `'adjustment'` |
| `created_at` | timestamptz | |

> Tidak pakai `is_deleted` — jangan filter. Tabel ini mungkin belum ada di fase awal; hook gracefully returns `[]` jika error.
...
| `reference_no` | text | nullable — nomor bukti transfer |
| `notes` | text | nullable |
| `created_at` | timestamptz | |
...
| `commission_rate` | numeric | persen komisi dari penjualan (jika salary_type = komisi) |
| `join_date` | date | nullable |
| `is_active` | boolean | default true |
| `is_deleted` | boolean | |
| `created_at` | timestamptz | |
...
> ⚠️ JANGAN INSERT `total_pay` (generated column). Tidak pakai `is_deleted`.
...
| `email` | text | NOT NULL |
| `name` | text | nullable |
| `interest` | text | sub-type atau nama fitur yang diminati |
| `created_at` | timestamptz | |
